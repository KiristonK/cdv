...
if (!empty($_GET['add'])){
    if (!empty($_POST['name']) && !empty($_POST['date'])) {
	    ...
	    if ($timeS <= $timeE) {
		    ...
		    if ($timeE != "00:00:00") $sql = "select * from scalendar.events where date = ? and time_stop > ? and time_start < ? and user_id = ?";
		    ...
		    echo "Event can not start after it's end. Start time must be earlier than end time.";
	    }
    } else {
